...
const FIELD =
  "w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-900 placeholder-gray-400 outline-none transition-all focus:border-transparent focus:bg-white focus:ring-2 focus:ring-teal-600";
const LABEL =
  "flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-gray-500 mb-1.5";
...
const Addhealthpost = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!category) {
      setErrors((prev) => ({
        ...prev,
        category: "Please choose a specialty category",
      }));
      return;
    }
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/addHealthPost`,
        {
          method: "POST",
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            <div>
              <label className={LABEL}>
                <User className="w-3.5 h-3.5" /> Doctor name
              </label>
              <input
                required
                name="doctorName"
            ...
            <div>
              <label className={LABEL}>
                <HeartPulse className="w-3.5 h-3.5" /> Specialty category
              </label>
              <div className="grid grid-cols-4 gap-2">
                {CATEGORIES.map(({ value, label, icon: Icon }) => {
                  ...
                  return (
                    ...
                    >
                      <Icon
                        className={`w-4.5 h-4.5 ${active ? "text-teal-700" : "text-gray-500"}`}
                      />
                      <span
                        className={`text-[11px] font-semibold ${active ? "text-teal-800" : "text-gray-600"}`}
                      >
                        {label}
                      </span>
                    </button>
