      ...
      >
        <div className="relative h-72 overflow-hidden">
          ...
          <span className="absolute top-4 right-4 flex h-12 w-12 items-center justify-center rounded-full bg-red-600 text-white shadow-lg ring-4 ring-white/30">
            <span className="text-[11px] font-bold leading-none">${doctor.price}</span>
          </span>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-5 translate-y-1 opacity-95">
            ...
            <p className="text-sm text-white/80 mt-0.5">{doctor.hospital}</p>
          </div>
        </div>
        ...
        <div className="p-6 pt-5 space-y-5">
          <div className="grid grid-cols-3 gap-2">
            <div className="rounded-2xl bg-slate-50 px-3 py-3 text-center transition-colors duration-300 group-hover:bg-teal-50">
              <svg className="mx-auto mb-1 h-4 w-4 text-teal-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
                <circle cx="12" cy="12" r="9" />
                <path d="M12 7v5l3 3" strokeLinecap="round" />
              </svg>
              <p className="text-[11px] text-slate-500">Experience</p>
              <p className="text-sm font-bold text-slate-800">{doctor.experience}y</p>
            </div>
            ...
            <div className="rounded-2xl bg-slate-50 px-3 py-3 text-center transition-colors duration-300 group-hover:bg-teal-50">
              ...
              <p className="truncate text-sm font-bold text-slate-800">{doctor.location}</p>
            </div>

            <div className="rounded-2xl bg-slate-50 px-3 py-3 text-center transition-colors duration-300 group-hover:bg-teal-50">
              <svg className="mx-auto mb-1 h-4 w-4 text-red-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
              ...
              <p className="text-sm font-bold text-red-600">${doctor.price}</p>
            </div>
          </div>
